...
public class ControllerManagerSchede implements UserRequest_IF, TrainerRequest_IF {
	...
	public static ControllerManagerSchede getController() {
		if (controller == null)
			controller = new ControllerManagerSchede();
		return controller;
	}
	...
	List<Esercizio> AlgoritmoGreedy(SchedaFitness s, List<Macchinario> lm) {
		...
		for (int i = 0; i < s.getLunghezzaLista(); i++) {
			...
			if (M != null) {
				aggiornata.add(new Esercizio(M.getNome(), M.getTipo(), E.getSerie()));
				continue;
			}
			...
			case "Bicipiti":
				X = bicipiti.poll();
				if (X == null || X.getTempo().equals("23:59:59")) {
					aggiornata.add(E);
					break;
				} else {
					aggiornata.add(new Esercizio(X.getNome(), X.getTipo(), E.getSerie()));
					break;
				}
